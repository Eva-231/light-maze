...
export function effectStats(stats){const s=stats;s.powers??=[];s.curses??=[];s.synergies=awakened(s.powers).map(a=>a.key);s.lightDrain=1;s.lootQuality=0;s.spawnFactor=1;s.normalHeal=1;s.castSpeed=1;if(has(s,'greedlamp')){s.lootQuality+=.25;s.lightDrain*=1.2;}
...
export function damageEffects(s,p,amount,random=Math.random){const m=p.effectState??={};let damage=amount;const hp=p.hp/s.maxHp;
 ...
 if(has(s,'manaarmor')){const spent=Math.min(p.mp,Math.ceil(damage*.4));p.mp-=spent;damage-=spent;}if(has(s,'substitute')&&hp>=.5&&damage>=p.hp&&p.elapsed>=(m.substituteAt||0)){m.substituteAt=p.elapsed+30;damage=Math.max(0,p.hp-1);}m.lastDamage=p.elapsed;return{damage:Math.max(0,damage)};
}
...
export function killEffects(s,p,e){const m=p.effectState??={};m.devoured=Math.min(300,(m.devoured||0)+(e.level||1));const multiplier=syn(s,'nightfeast')?2:1,heal=((has(s,'leech')?5:0)+(s.boonLeech||0)+(s.enchantLeech||0)+(cursed(s,'bloodamulet')?7:0)+(syn(s,'deathline')&&p.hp/s.maxHp<.3?8:0))*multiplier*(s.recoveryScale||1);p.hp=Math.min(s.maxHp,p.hp+heal);if(has(s,'devourlight')||s.enchantLight)p.light=Math.min(p.lightMax,p.light+(has(s,'devourlight')?3*multiplier:0)+(s.enchantLight||0));if(has(s,'plunder'))p.bag+=100;if(has(s,'procession'))p.bag+=64;}
