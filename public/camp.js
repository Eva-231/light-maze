...
export function createCamp({getProgress,changed,sound,openDialog,rankings,canChange=()=>true,onLesson=()=>{}}){
 ...
 function render(){summary();const unlocked=getProgress().floor>=4;document.querySelectorAll('[data-camp-tab]').forEach(b=>{if(b.dataset.campTab==='forge')b.hidden=!unlocked;});for(const b of document.querySelectorAll('[data-camp-tab]'))b.setAttribute('aria-selected',String(b.dataset.campTab===tab));
  ...
  if(tab==='forge'){
   ...
   $('odds-info').textContent=p.pity>=9?'今回の排出率：特級99%・伝説1%':'通常62.5%・希少29%・特級7.5%・伝説1%';
   $('cache-info').textContent=p.caches.length?'次の遺物：'+(p.caches[0].minTier?(p.caches[0].forcedType!==undefined?RELICS[p.caches[0].forcedType].name:'突破・撃破報酬')+' / '+RARITIES[p.caches[0].minTier].ja+(p.caches[0].minTier===3?'確定':'以上確定'):['通常鉱脈（伝説1%）','希少鉱脈（伝説1%）','金の鉱脈（伝説1%・深淵は深度で上昇）'][p.caches[0].quality]):'壁の鉱脈・高位の宝・敵の撃破から発見できます。生還して持ち帰りましょう。';
  }
  if(tab==='loadout'){
   const bestTier=Math.max(0,...p.equipped.map(id=>p.inventory.find(i=>i.id===id)?.tier??0));$('avatar-rarity').textContent=RARITIES[bestTier].name;$('avatar-rarity').style.color=RARITIES[bestTier].color;const stats=loadoutStats(p);$('synergy-hints').innerHTML=synergyStatus(p).filter(s=>s.count>0).map(s=>'<p>'+ (s.known?s.name:'未知の共鳴')+' '+s.count+'/3 · '+(s.count===3?'覚醒中：'+s.effect:s.known?'あと '+s.parts.filter(a=>!a.equipped).map(a=>a.name).join('・')+' を装備すると覚醒':'条件は初覚醒で公開')+'</p>').join('');$('workshop-balance').textContent='星屑 '+(p.shards||0)+' · 分解5/12/30、再抽選10、1付加性能固定20';$('loadout-stats').textContent='HP '+stats.maxHp+' · MP '+stats.maxMp+' · 魔法 ×'+stats.spellMultiplier+' / 魔力 +'+stats.power+' · 防御 '+stats.armor+' · 移動 ×'+stats.moveSpeed+(stats.warpImmune?' · 転位無効':'');
  ...
  if(tab==='records'){
   ...
   $('history-list').innerHTML=p.history.length?p.history.map(h=>`<div class="history-row"><div><strong>${h.score.toLocaleString()}<small>点</small></strong><span>${h.mode==='daily'?'日替わり':h.mode==='abyss'?'深淵'+h.abyssFloor+'F':h.floor===8&&h.trialLevel>0?'旧試練Lv'+h.trialLevel:h.floor+'面'} · ${h.cleared?'突破':h.success?'途中帰還':'LOST'}</span></div><div><strong>${h.kills} 撃破</strong><span>${h.time.toFixed(2)}秒 · ${h.rules===8?'連戦'+(h.maxHuntChain||0)+' / 反撃'+(h.counterHits||0):'旧方式'}</span></div></div>`).join(''):'<p class="empty-note">最初の探索が、あなたの記録になります。</p>';
  }
 }
 ...
 $('camp-dialog').addEventListener('click',e=>{
  ...
  if(b.dataset.equip||b.id==='equip-new'){const i=p.inventory.find(i=>i.id===(b.dataset.equip||last?.id));if(!i)return;p.equipped[RELICS[i.type].slot]=i.id;const awakened=discoverLoadout(p);if(awakened.length)onLesson('abyss-synergy','シナジー覚醒',awakened.map(s=>s.name+'：'+s.effect).join(' / ')+'。必要な能力を同時に装備すると、専用能力に進化します。');changed();sound.chime(0);if(b.id==='equip-new')tab='loadout';render();if(b.id==='equip-new'){b.textContent='装備しました';b.disabled=true;}}
  if(b.dataset.work){const r=workshop(p,b.dataset.item,b.dataset.work,crypto.getRandomValues(new Uint32Array(1))[0],!!b.dataset.lock);if(r.error){$('inventory-message').textContent=r.error;return;}changed();render();$('inventory-message').textContent=r.message;return;}if(b.dataset.sell){const result=sellRelic(p,b.dataset.sell);if(result.error){$('inventory-message').textContent=result.error;return;}changed();render();}
 });
 return{open,render};
}
