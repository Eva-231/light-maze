...
export function generateStage(seed=1,floor=1,trialLevel=1,daily=false,options={}){
 ...
 const idealSeconds=stageInfo.optimalSteps*S/SPEED;stageInfo.baseDrain=config.rules?.bonus?0:clamp(64/(idealSeconds*1.45+22),.22,.50)*config.lightScale*(1+Math.min(.45,Math.max(0,config.trialLevel-1)*.025));stageInfo.parTime=Math.ceil(idealSeconds*1.45+config.enemies*5+config.seals*2+12);return stageInfo;
}
...
export function orbPlan(){const available=rooms.slice(1).sort((a,b)=>distances[a.z*W+a.x]-distances[b.z*W+b.x]),bonus=!!stageInfo.config.rules?.bonus,count=bonus?available.length:Math.min(available.length,stageInfo.config.rules?.boss?12:stageInfo.floor<=2?3:5),selected=[];for(let i=0;i<count;i++){const r=bonus?available[i]:available[Math.floor(i*(available.length-1)/Math.max(1,count-1))];selected.push({x:r.x*S-.6,z:r.z*S+.55,collected:false,amount:bonus?35:stageInfo.config.rules?.boss?28:stageInfo.floor<=2?16:18});}return selected;}
