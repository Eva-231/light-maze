...
export function rollRelic(seed,quality=0,pity=0,options={}){
 ...
 const unlocked=options.unlocked||18,allowed=RELICS.map((_,i)=>i).filter(i=>tier>=4?RELICS[i].abyss&&!!RELICS[i].mythic===(tier===5)&&(RELICS[i].minFloor||10)<=(options.abyssFloor||99):!RELICS[i].exclusive&&TYPE_UNLOCKS[i]<=unlocked);
 ...
 const a=Math.floor(r()*AFFIXES.length),b=(a+1+Math.floor(r()*(AFFIXES.length-1)))%AFFIXES.length,minRoll=Math.max(1,Math.min(100,options.minRoll||depth?.minRoll||1));
 ...
 return{id:(seed>>>0).toString(36)+'-'+Math.floor(r()*1e8).toString(36),type,tier,a,b,rollA:minRoll+Math.floor(r()*(101-minRoll)),rollB:minRoll+Math.floor(r()*(101-minRoll)),depth:Math.max(0,Math.min(20,Math.floor(options.depth||depth?.powerDepth||0))),favorite:false,...(def.abyss?{extraPowers,upgrade:0,evolutionKills:0,evolved:false}: {})};
}
export function affixValue(item,second=false){const depth=Math.max(0,Math.min(20,item.depth||0)),scale=1+depth*.025;return Math.max(1,Math.round(AFFIXES[second?item.b:item.a].max*[.55,.90,1.45,2.6,3.5,4.8][item.tier]*(1+(item.upgrade||0)*.04)*(.45+(second?item.rollB:item.rollA)/100*.55)*scale));}
...
export function workshop(progress,id,action,seed=1,lock=false){const item=progress.inventory.find(i=>i.id===id);if(!item||item.tier<3)return{error:'伝説以上の装備を選んでください。'};progress.shards??=0;
 ...
 if(action==='upgrade'){if((item.upgrade||0)>=5)return{error:'強化上限 +5 です。'};item.upgrade=(item.upgrade||0)+1;}else if(action==='reroll'){const r=random(seed);if(!lock){item.a=Math.floor(r()*AFFIXES.length);item.rollA=1+Math.floor(r()*100);}item.b=(item.a+1+Math.floor(r()*5))%6;item.rollB=1+Math.floor(r()*100);if(RELICS[item.type].abyss&&!lock){const pool=POWERS.filter(p=>p.minFloor<=Math.max(10,item.depth*5)&&!RELICS[item.type].powers?.includes(p.key));if(pool.length)item.extraPowers=[pool[Math.floor(r()*pool.length)].key];}}else return{error:'不明な操作です。'};progress.shards-=cost;return{message:action==='upgrade'?'性能を強化しました':'付加効果を再抽選しました（固有能力は保持）'};
}
