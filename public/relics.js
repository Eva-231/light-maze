...
export const RARITIES=[{name:'COMMON',ja:'通常',color:'#b0ced1'},{name:'RARE',ja:'希少',color:'#b7a1ff'},{name:'EPIC',ja:'特級',color:'#ff9bd9'},{name:'LEGENDARY',ja:'伝説',color:'#ffdc83'}];
export const RELICS=[
...
export function rarityOdds(quality=0,pity=0){return pity>=9?[0,0,.9,.1]:quality>=2?[.08,.42,.40,.10]:quality===1?[.35,.40,.21,.04]:[.60,.28,.10,.02];}
...
export function appraisal(progress,seed,{cache=false}={}){
 ...
 progress.inventory.push(item);const key=item.type*4+item.tier,isNew=!progress.catalog.includes(key);if(isNew)progress.catalog.push(key);
...
export function claimMilestoneGifts(progress){
 const received=[];for(const c of CHAPTERS){if(c.gift===undefined||!progress.chapterStars[String(c.id)]||progress.claimedGifts.includes(c.id)||progress.inventory.length>=INVENTORY_LIMIT)continue;
  const item=rollRelic(981237+c.id*377,0,0,{forcedType:c.gift,minTier:c.id===7?3:1});item.id='gift-'+c.id;progress.inventory.push(item);progress.claimedGifts.push(c.id);const key=item.type*4+item.tier;if(!progress.catalog.includes(key))progress.catalog.push(key);const slot=RELICS[item.type].slot;if(!progress.equipped[slot])progress.equipped[slot]=item.id;received.push(item);
 }return received;
}
