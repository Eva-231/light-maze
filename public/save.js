import {freshAbyss} from './abyss-run.js';
export const newProgress=()=>({format:8,abyss:freshAbyss(),codex:{equipment:[],grades:[],enchants:[],mythic:[],effects:[],synergies:[],enemies:[],mutations:[],traps:[],excavations:[],evolutions:[]},shards:0,playerName:'',difficultyStats:{},chapterStars:{},claimedGifts:[],hintsSeen:[],trialLevel:1,bestTrial:0,bestSkill:0,mask:'',completed:[],forgeSpent:0,inventory:[],equipped:['','',''],caches:[],catalog:[],pity:0,draws:0,totalKills:0,bestScore:0,history:[],dailyBests:[],totalBank:0,level:0,runs:0,best:0,deepest:0,escapes:0,busts:0,floor:1,clears:0,bestCoverage:0,bestClearTime:0});
export const migrateProgress=data=>{const migrated={...newProgress(),...data,format:8,mask:'',completed:[]};migrated.floor=Math.max(1,Math.min(18,Number(migrated.floor)||1));migrated.abyss={...freshAbyss(),...data?.abyss};migrated.abyss.unlocked=!!migrated.chapterStars?.['18'];migrated.codex={...newProgress().codex,...data?.codex};migrated.codex.equipment=[...(migrated.codex.equipment||[])];if((Number(data?.format)<8||!data?.codex)&&Array.isArray(migrated.catalog))for(const key of migrated.catalog){const type=key>=1000?key-1000:Math.floor(key/4);if(type>=0&&type<96&&!migrated.codex.equipment.includes(type))migrated.codex.equipment.push(type);}migrated.shards=Math.max(0,Number(migrated.shards)||0);migrated.playerName=typeof migrated.playerName==='string'?migrated.playerName.trim().slice(0,12):'';migrated.inventory=(migrated.inventory||[]).map(item=>({...item,favorite:!!item.favorite}));return migrated;};
...
const usableGuest=data=>data&&typeof data==='object'&&!Array.isArray(data)&&[1,2,3,4,5,6,7,8].includes(data.format)&&Number.isFinite(data.totalBank)&&(data.format<3||Array.isArray(data.inventory)&&Array.isArray(data.caches));
...
export class Save{
 ...
 async load(){
  ...
  const result=await response.json();if(!result||typeof result!=='object'||Array.isArray(result)||(Object.keys(result).length&&(![1,2,3,4,5,6,7,8].includes(result.format)||!Number.isFinite(result.totalBank)))){this.notify('error');throw new Error('Invalid saved progress');}
