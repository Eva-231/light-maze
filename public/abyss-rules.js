...
export function population(depth=1){const f=bound(depth,1,99);return f>=95?{initial:14,spawn:2}:f>=80?{initial:13,spawn:2}:f>=65?{initial:12,spawn:2}:f>=50?{initial:11,spawn:2}:f>=35?{initial:10,spawn:1}:f>=20?{initial:9,spawn:1}:f>=10?{initial:8,spawn:1}:{initial:7,spawn:1};}
...
export const ROUTES=[{id:'normal',name:'通常の回廊',text:'通常の危険度と報酬',enemy:1,quality:0,traps:1},{id:'hunt',name:'修羅の回廊',text:'初期敵 +35%（最大20）・品質 +20%',enemy:1.35,quality:.20,traps:1},{id:'trap',name:'埋もれた回廊',text:'罠 +60%・発掘率と品質上昇',enemy:1,quality:.12,traps:1.6}];
export const MUTATORS=[{id:'wrath',name:'敵攻撃 +50%'},{id:'horde',name:'増援60秒'},{id:'famine',name:'HP回復半減'},{id:'elite',name:'エリート大量'},{id:'night',name:'LIGHT消費 +40%'}];
export function floorRules(floor,{route='normal',mutators=[]}={}){const f=bound(floor,1,99),r=ROUTES.find(r=>r.id===route)||ROUTES[0],pop=population(f),bonus=isBonusFloor(f),night=f>=20&&f<40||f>=60&&f<80,horde=f>=40&&f<60||f>=80,forcedHorde=mutators.includes('horde');return{floor:f,bonus,bonusCount:bonus?(f===25?1:f===50?2:f===75?3:1):0,route:r.id,level:enemyLevelForFloor(f,{endless:true}),initial:bonus?0:Math.min(20,Math.ceil(pop.initial*r.enemy)),spawnCount:bonus?0:pop.spawn,spawnInterval:horde||forcedHorde?30:60,spawnDelay:forcedHorde?60:horde?75:120,light:bonus?0:(night?1.3:1)*(mutators.includes('night')?1.4:1),damage:mutators.includes('wrath')?1.5:1,heal:mutators.includes('famine')?.5:1,elite:(horde?2:1)*(mutators.includes('elite')?3:1),quality:r.quality+(night?.4:0)+mutators.length*.08,traps:bonus?0:r.traps,scoreMultiplier:1+mutators.length*.25,curse:bonus?'':night?'永夜：LIGHT +30%・品質 +40%':horde?'百鬼夜行：増援75秒・エリート率×2':'',boss:BOSS_FLOORS.includes(f),chest:f%5===0||bonus};}
