...
export function floorRules(floor,{route='normal',mutators=[]}={}){const f=bound(floor,1,99),r=ROUTES.find(r=>r.id===route)||ROUTES[0],pop=population(f),bonus=isBonusFloor(f),night=f>=20&&f<40||f>=60&&f<80,horde=f>=40&&f<60||f>=80,forcedHorde=mutators.includes('horde');return{floor:f,bonus,bonusCount:bonus?(f===25?1:f===50?2:f===75?3:1):0,route:r.id,level:enemyLevelForFloor(f,{endless:true}),initial:bonus?0:Math.min(20,Math.ceil(pop.initial*r.enemy)),spawnCount:bonus?0:pop.spawn,spawnInterval:forcedHorde?60:horde?75:90,light:bonus?0:(night?1.3:1)*(mutators.includes('night')?1.4:1),damage:mutators.includes('wrath')?1.5:1,heal:mutators.includes('famine')?.5:1,elite:(horde?2:1)*(mutators.includes('elite')?3:1),quality:r.quality+(night?.4:0)+mutators.length*.08,traps:bonus?0:r.traps,scoreMultiplier:1+mutators.length*.25,curse:bonus?'':night?'永夜：LIGHT +30%・品質 +40%':horde?'百鬼夜行：増援75秒・エリート率×2':'',boss:BOSS_FLOORS.includes(f),chest:f%5===0||bonus};}
