...
const powerColor=p=>p.some(k=>['iceburial','deepfreeze'].includes(k))?0x83d8ff:p.some(k=>['corpse','blast'].includes(k))?0xffb871:p.includes('leech')?0xff91b6:p.includes('pristine')?0xf4ffed:p.includes('cycle')?0xbd98ff:null;
export function gearTint(stats){return (stats.curses||[]).length?0xa769d8:powerColor(stats.powers||[])??new T.Color(RARITIES[stats.staffTier||0].color).getHex();}
export function gearOrnaments(item,tier){const group=new T.Group(),d=RELICS[item?.type||0],color=item?.evolved?0xffcf6f:d.curse?0xa665de:powerColor(d.powers||[])??new T.Color(RARITIES[tier].color).getHex(),mat=new T.MeshStandardMaterial({color,emissive:color,emissiveIntensity:tier>=5?2.2:1+gearGrade(item||{})*.15,metalness:.7,roughness:.2});for(let i=0;i<Math.min(4,Math.max(1,tier-1));i++){const ring=new T.Mesh(new T.TorusGeometry(.2+i*.07+gearGrade(item||{})*.012,.01,4,32),mat);ring.position.y=2.1+i*.12;ring.rotation.x=(item?.type||0)%3*.65;ring.rotation.z=i*.8;group.add(ring);}if(tier===5){const star=new T.Mesh(new T.OctahedronGeometry(.12),mat);star.position.set(.6,2.25,0);star.scale.y=d.key==='eater'?2.5:d.key==='amaterasu'?1.4:1;group.add(star);if(item?.evolved){for(let i=0;i<3;i++){const crown=new T.Mesh(new T.OctahedronGeometry(.07),mat);crown.position.set((i-1)*.2,2.7+(.1*(i%2)),0);crown.scale.y=2;group.add(crown);}}}return group;}
