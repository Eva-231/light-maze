...
export class RevealSequence {
 ...
 next(){this.index++;if(this.index>=this.results.length){this.finish();return;}const index=this.index,tier=this.results[index].item.tier;
  ...
  // Chain confirmation, opening and hold so a slow frame cannot consume the rare hold.
  const burst=()=>{this.emit({phase:'burst',index,tier,result:this.results[index]});this.after(hold,()=>this.next());};
  if(tier>=2){this.after(900,()=>this.emit({phase:'charge',index}));if(tier>=3)this.after(3000,()=>this.emit({phase:'silence',index}));const promiseAt=tier>=5?5700:tier===4?4600:tier===3?4000:2400;this.after(promiseAt,()=>{this.emit({phase:'promise',index,tier});this.after(revealAt-promiseAt,burst);});}
  else this.after(revealAt,burst);
 }
