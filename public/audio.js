...
export class Sound{
 ...
 ensureTrack(key){
  if(this.tracks.has(key))return this.tracks.get(key);const meta=MUSIC_TRACKS[key]||MUSIC_TRACKS.home,audio=new Audio(meta.src);audio.loop=true;audio.preload='metadata';audio.playsInline=true;const gain=this.context.createGain();gain.gain.value=0;const source=this.context.createMediaElementSource(audio);source.connect(gain);gain.connect(this.musicFilter);const channel={audio,gain,source,key};this.tracks.set(key,channel);return channel;
 }
 ...
 async start(){
  try{
   ...
   const ctx=this.context;this.setEnabled();await Promise.allSettled([ctx.state==='suspended'?ctx.resume():Promise.resolve(),this.settings.sound&&this.settings.musicVolume!==0?this.ensureTrack(this.currentKey).audio.play():Promise.resolve()]);
 ...
 setEnabled(){if(!this.context)return;const t=this.context.currentTime;this.master.gain.cancelScheduledValues(t);this.master.gain.setTargetAtTime(this.settings.sound?.6:0,t,.08);if(!this.settings.sound||this.settings.musicVolume===0)for(const channel of this.tracks.values())channel.audio.pause();}
 ...
 chime(tier=0){const f=[659.25,880,1108.73,1318.51][tier];this.note(f,.34,.08);this.note(f*1.5,.50,.035,.05);if(tier>0){this.note(f*1.2599,.42,.05,.11);this.note(f*2,.55,.045,.11);}}
