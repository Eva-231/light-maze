export const CHAPTERS=[
 ...
 {id:8,name:'深淵試練',lesson:'装備を組み、最高到達Lvを更新する',cols:4,rows:4,seals:3,enemies:9,kinds:[0,1,2,3,4,5],spells:[0,1,2],flash:true,mine:true,traps:true,collapse:true,omens:true,lightScale:1.05,reward:'毎回 伝説2個（深淵専用1個）· Lv5ごと伝説3個'}
];
...
export function clearRewardTokens(run){
 ...
 const count=run.floor===7?2:run.trialLevel%5===0?3:2;
 return Array.from({length:count},(_,i)=>({seed:((run.seed^0x7F83A)+Math.imul(i+1,2654435761))>>>0,quality:2,minTier:run.floor===8?3:2,minRoll:run.floor===8?70:40,...(run.floor===8&&i!==1?{forcedType:18+((run.trialLevel-1+i)%3)}:{})}));
}
...
export function chapterConfig(stage=1,trialLevel=1){const chapter=CHAPTERS[Math.max(0,Math.min(7,Math.floor(stage)-1))];return{...chapter,difficulty:difficultyFor(stage),trialLevel:stage>=8?Math.max(1,trialLevel):0};}
