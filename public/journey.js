...
export function abyssConfig(floor=1,options={}){const rules=floorRules(floor,options),endgame=floor>=90?1+Math.min(10,floor-89)*.015:1,endDamage=floor>=90?1+Math.min(10,floor-89)*.009:1;return{...CHAPTERS[17],id:18,name:rules.bonus?'★ BONUS FLOOR · 深淵 '+floor+'F':'深淵 '+floor+'F',abyss:true,abyssFloor:floor,cols:4,rows:4,seals:3,enemies:rules.initial,kinds:rules.boss?[0,1,2,4,5]:[0,1,2,4],collapse:false,omens:false,traps:!rules.bonus,lightScale:1,trialLevel:0,enemyLevel:rules.level,rules,difficulty:{...DIFFICULTIES[1],key:'abyss',name:rules.bonus?'BONUS FLOOR':'深淵',target:rules.bonus?'安全階 · 敵/罠なし · LIGHT消費なし':floor>=90?'Grade V基準 · MYTHICで余裕':'深度で変化',hp:1.25*endgame,damage:1.3*endDamage,speed:1.08,windup:.85,recovery:.82,trapFuse:.45,trapDamage:24+rules.level*4}};}
