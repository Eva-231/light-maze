...
export function createAbyssUI({getProgress,getRun,openDialog,onBegin,onGate,onBoon,onEvent}){
 ...
 if(ABYSS_TEST_MODE){queueMicrotask(()=>{const open=()=>{const p=getProgress();if(!canEnterAbyss(p))return setTimeout(open,250);$('abyss-open').click();};setTimeout(open,350);});}
 return{
