...
function testPanel(){return `<div class="test-floor-links" style="display:grid;gap:12px"><div><strong>TEST PLAY · 開始階層</strong><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px">${testFloors.map(f=>`<button type="button" data-test-floor="${f}" class="${f===ABYSS_TEST_FLOOR?'selected':''}">${f}F</button>`).join('')}</div></div><div><strong>装備プリセット</strong><div style="display:grid;gap:6px;margin-top:8px">${testGears.map(([id,name,text])=>`<button type="button" data-test-gear="${id}" class="${id===ABYSS_TEST_GEAR?'selected':''}"><strong>${name}</strong><small>${text}</small></button>`).join('')}</div></div><div><strong>追加対策</strong><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px"><button type="button" data-test-warp="0" class="${!ABYSS_TEST_WARP?'selected':''}">転移対策なし</button><button type="button" data-test-warp="1" class="${ABYSS_TEST_WARP?'selected':''}"><strong>定着</strong><small>転位術を完全無効化</small></button></div></div><p style="margin:0">おすすめ：<b>90F × 深淵 Grade V</b>、<b>98F × MYTHICあり</b>。転移対策はテスト専用の追加条件です。テスト結果は通常進行・戦利品・ランキングへ反映しません。</p></div>`;}
function testUrl(floor=90,gear='abyss5'){const u=new URL(location.href);u.searchParams.set('test','abyss');u.searchParams.set('floor',String(floor));u.searchParams.set('gear',gear);u.searchParams.set('v','abyss-test-5');return u.toString();}
function replaceTestQuery(key,value){const u=new URL(location.href);u.searchParams.set('test','abyss');u.searchParams.set(key,String(value));u.searchParams.set('v','abyss-test-5');location.href=u.toString();}
...
export function createAbyssUI({getProgress,getRun,openDialog,onBegin,onGate,onBoon,onEvent}){
 ...
 return{
 gate(){const r=getRun();$('abyss-gate-title').textContent=(r.testMode?'TEST · ':'')+'深淵 '+r.floor+'F · 3封印解除';$('abyss-gate-status').textContent=(r.testMode?'テストプレイ'+(r.testWarp?' / 転移無効':'')+' / ':'')+'戦利品 '+r.caches.length+'個 / ¥'+Math.floor(r.bag).toLocaleString()+(r.testMode?' · 結果は保存されません':' · 帰還すると確定');const routes=r.floor%3===0?ROUTES:ROUTES.slice(0,1);$('abyss-gate-options').innerHTML=(r.floor<99?routes.map(a=>`<button data-gate="next" data-route="${a.id}"><strong>${a.name} → ${r.floor+1}F</strong><small>${a.text||'次の迷宮へ進む'}</small></button>`).join(''):'<p>99F踏破。テスト結果は通常進行へ反映しません。</p>')+(r.testMode?'':'<button data-gate="suspend">中断セーブしてホームへ</button>')+'<button data-gate="return">'+(r.testMode?'テストを終了':'帰還して戦利品を確保')+'</button>';$('abyss-gate-close').hidden=false;openDialog('abyss-gate-dialog');},
