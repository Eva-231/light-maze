...
function testPanel(){return `<div class="test-floor-links" style="display:grid;gap:12px"><div><strong>TEST PLAY · 開始階層</strong><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px">${testFloors.map(f=>`<button type="button" data-test-floor="${f}" class="${f===ABYSS_TEST_FLOOR?'selected':''}">${f}F</button>`).join('')}</div></div><div><strong>装備プリセット</strong><div style="display:grid;gap:6px;margin-top:8px">${testGears.map(([id,name,text])=>`<button type="button" data-test-gear="${id}" class="${id===ABYSS_TEST_GEAR?'selected':''}"><strong>${name}</strong><small>${text}</small></button>`).join('')}</div></div><div><strong>追加対策</strong><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px"><button type="button" data-test-warp="0" class="${!ABYSS_TEST_WARP?'selected':''}">追加の転移対策なし</button><button type="button" data-test-warp="1" class="${ABYSS_TEST_WARP?'selected':''}"><strong>定着を追加</strong><small>プリセットに関係なく転位術を完全無効化</small></button></div></div><p style="margin:0">Grade IVは標準で定着エンチャント入り。ほかのプリセットにも実戦想定のエンチャントを付与しています。おすすめ：<b>80F × Grade IV</b>、<b>90F × Grade V</b>、<b>98F × MYTHIC</b>。</p></div>`;}
function testUrl(floor=90,gear='abyss5'){const u=new URL(location.href);u.searchParams.set('test','abyss');u.searchParams.set('floor',String(floor));u.searchParams.set('gear',gear);u.searchParams.set('v','abyss-test-6');return u.toString();}
function replaceTestQuery(key,value){const u=new URL(location.href);u.searchParams.set('test','abyss');u.searchParams.set(key,String(value));u.searchParams.set('v','abyss-test-6');location.href=u.toString();}
