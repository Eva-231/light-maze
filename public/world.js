...
export class World{
 constructor(canvas,reveal,settings){
  this.reveal=reveal;this.settings=settings;this.elapsed=0;this.bloomKick=0;this.hitShake=0;this.loot=[];this.orbs=[];this.sealMeshes=[];this.enemyMeshes=[];this.veinMeshes=[];this.trapMeshes=[];this.supplyMeshes=[];this.beamFX=[];this.dropMeshes=[];this.actorEnemies=[];this.animations=[];this.obstacles=[];this.particles=[];this.qualityScale=1;this.frameBudget=[];
 ...
 resetLevel(reveal,items,orbs,relics,enemies,expedition=null){
  ...
  this.loot=[];this.orbs=[];this.sealMeshes=[];this.enemyMeshes=[];this.veinMeshes=[];this.trapMeshes=[];this.supplyMeshes=[];this.beamFX=[];this.dropMeshes=[];this.actorEnemies=[];this.animations=[];this.obstacles=[];this.particles=[];this.jackBeam=null;
 ...
 buildEnemies(enemies){
  ...
  for(const enemy of enemies){const group=new T.Group(),kind=enemy.kind,accent=new T.MeshBasicMaterial({color:ENEMY_TYPES[kind].color}),armor=new T.MeshStandardMaterial({color:kind===5?0x8f7550:kind===4?0xc2d4dc:0x29343b,metalness:kind===4?.95:.7,roughness:.28}),legs=[];
   ...
   this.enemyMeshes.push({enemy,group,eyes:accent,aura,body,legs,telegraph});
  }
 }
 spawnEnemies(additions,all){this.buildEnemies(additions);this.actorEnemies=all;}
 ...
 setEquipment(stats){this.equipment=stats;if(!this.focusWeapon)return;this.staffCore.material=this.glowMaterials[stats.staffTier||0];this.staffCrown.visible=(stats.staffTier||0)>0;this.staffCrown.material=this.glowMaterials[stats.staffTier||0];this.focusWeapon.scale.setScalar(1+(stats.staffType>=15?.14:0));}
