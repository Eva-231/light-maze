...
export function castSpell(index,enemies,player,stats,random=Math.random){
 ...
 let bounced=[];const selected=index===0?targets.slice(0,has(stats,'pierce')||has(stats,'drill')?3:has(stats,'twinstar')?2:1):index===1?targets.slice(0,2):targets;if(index===0&&stats.echo&&selected.length){const first=selected[0],echo=enemies.filter(e=>!e.dead&&!e.escaped&&e!==first&&Math.hypot(e.x-first.x,e.z-first.z)<5&&lineOfSight(first.x,first.z,e.x,e.z)).slice(0,stats.echoCount||1);selected.push(...echo);}
