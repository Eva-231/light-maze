...
let enemySerial=0;
export function enemyPlan(extra=0){enemySerial=0;
...
export function castSpell(index,enemies,player,stats,random=Math.random){
 ...
 const cone=has(stats,'homing')?.5:stageInfo.floor<=2?.30:stageInfo.floor<=4?.23:.17,targets=enemies.filter(e=>!e.dead&&!e.escaped&&Math.hypot(e.x-player.x,e.z-player.z)<(index===1?11:12)&&lineOfSight(player.x,player.z,e.x,e.z)&&(index===2||facing(player,e.x,e.z,index===1?.29:cone))).sort((a,b)=>Math.hypot(a.x-player.x,a.z-player.z)-Math.hypot(b.x-player.x,b.z-player.z));if(blood){player.hp-=cost-player.mp;player.mp=0;}else player.mp-=cost;if(has(stats,'lightpact'))player.light=Math.max(.1,player.light-1);const events=[];
 ...
 let bounced=[];const selected=index===0?targets.slice(0,has(stats,'pierce')||has(stats,'drill')?3:1):index===1?targets.slice(0,2):targets;if(index===0&&stats.echo&&selected.length){const first=selected[0],echo=enemies.filter(e=>!e.dead&&!e.escaped&&e!==first&&Math.hypot(e.x-first.x,e.z-first.z)<5&&lineOfSight(first.x,first.z,e.x,e.z)).slice(0,stats.echoCount||1);selected.push(...echo);}
 ...
 for(let i=0;i<selected.length;i++){const e=selected[i],interrupted=e.kind===3&&e.windup>0,weak=e.recovery>0||e.stun>.35||(e.kind!==5&&e.windup>0),critical=player.runMode!=='daily'&&random()<stats.crit,isCounter=counter&&i===0,frostFavored=index===1&&[1,4].includes(e.kind),affinity=(stats.enemyMultipliers?.[e.kind]||1)*(frostFavored?1.6:1);let damage=((index===0?28*stats.boltBonus:24)+stats.power)*(stats.spellMultiplier||1)*affinity;if(weak)damage*=1.5*(stats.weakBonus||1);else if((e.kind===2||e.kind===5)&&!has(stats,'insight'))damage*=e.kind===5?.40:.6;if(isCounter)damage*=1.5;if(critical)damage*=1.6;if(index===0&&i>0&&!has(stats,'pierce')&&!has(stats,'drill'))damage*=stats.echoDamage??.35;damage*=strikeMultiplier(stats,player,e,index,Math.hypot(e.x-player.x,e.z-player.z),i);if(bounced.length)damage*=1.5;if(index===1)e.ward=0;if(e.ward>0){e.ward--;damage*=.2;}if(e.mutations?.includes('phantom')&&e.stun<=0&&index!==1)damage*=.75;if(syn(stats,'nightfeast')&&player.light/player.lightMax<.2)e.ward=0;if(e.kind!==5&&((has(stats,'execute')&&e.hp/e.maxHp<.15)||(cursed(stats,'deathcontract')&&random()<.1)))damage=e.hp;
  const hit=hurtEnemy(e,damage);Object.assign(hit,{critical,weak,interrupted,counter:isCounter,affinity:affinity>1.01,frost:index===1});if(bounced.length)hit.bounce=bounced[0].bounce;events.push(hit);rememberStrike(stats,player,e);
 ...
 if(primary.length){
  const origin=primary[0].enemy,range=(has(stats,'amaterasu')?3+4*player.light/player.lightMax:3)*(stats.blastRange||1),rain=has(stats,'rain')&&(m.hits||0)%5===0;
  ...
  if(index===1&&syn(stats,'permafrost'))for(const e of enemies)if(!e.dead&&Math.hypot(e.x-origin.x,e.z-origin.z)<5)stopCast(e,e.kind===5?1:4);
