...
export const SPELLS=[{name:'光弾',key:'bolt',cost:6,cooldown:.65,description:'狙って放つ。外すとMPを消費。攻撃後の隙で威力上昇。'},{name:'氷環',key:'frost',cost:16,cooldown:4.2,description:'自分の周囲5.2mを凍結爆破。最大8体へダメージ＋足止め。囲まれた時の緊急脱出用。'},{name:'治癒',key:'heal',cost:20,cooldown:7,description:'HPを30回復。'}];
...
export function castSpell(index,enemies,player,stats,random=Math.random){
 ...
 let bounced=[];const selected=index===0?targets.slice(0,has(stats,'pierce')||has(stats,'drill')?3:has(stats,'twinstar')?2:1):index===1?targets.slice(0,8):targets;if(index===0&&stats.echo&&selected.length){const first=selected[0],echo=enemies.filter(e=>!e.dead&&!e.escaped&&e!==first&&Math.hypot(e.x-first.x,e.z-first.z)<5&&lineOfSight(first.x,first.z,e.x,e.z)).slice(0,stats.echoCount||1);selected.push(...echo);}
 ...
 for(let i=0;i<selected.length;i++){const e=selected[i],interrupted=e.kind===3&&e.windup>0,weak=e.recovery>0||e.stun>.35||(e.kind!==5&&e.windup>0),critical=player.runMode!=='daily'&&random()<stats.crit,isCounter=counter&&i===0,frostFavored=index===1&&[1,4].includes(e.kind),affinity=(stats.enemyMultipliers?.[e.kind]||1)*(frostFavored?1.6:1);let damage=((index===0?28*stats.boltBonus:20)+stats.power)*(stats.spellMultiplier||1)*affinity;if(weak)damage*=1.5*(stats.weakBonus||1);else if((e.kind===2||e.kind===5)&&!has(stats,'insight'))damage*=e.kind===5?.40:.6;if(isCounter)damage*=1.5;if(critical)damage*=1.6;if(index===0&&i>0&&!has(stats,'pierce')&&!has(stats,'drill')&&!has(stats,'twinstar'))damage*=stats.echoDamage??.35;damage*=strikeMultiplier(stats,player,e,index,Math.hypot(e.x-player.x,e.z-player.z),i);if(bounced.length)damage*=1.5;if(index===1)e.ward=0;if(e.ward>0){e.ward--;damage*=.2;}if(e.mutations?.includes('phantom')&&e.stun<=0&&index!==1)damage*=.75;if(syn(stats,'nightfeast')&&player.light/player.lightMax<.2)e.ward=0;if(e.kind!==5&&((has(stats,'execute')&&e.hp/e.maxHp<.15)||(cursed(stats,'deathcontract')&&random()<.1)||(has(stats,'death')&&random()<.08)))damage=e.hp;
  ...
  if(!e.dead){e.memory=e.kind===5?10:6;e.target=indexAt(player.x,player.z);if(index===1||interrupted){stopCast(e,e.kind===5?.65:interrupted?1.8:(frostFavored?4.2:2.6)+stats.freezeBonus);e.attackCooldown=Math.max(e.attackCooldown,2);if(e.kind===1)e.charge=0;}else if(e.kind===0||e.kind===4){e.stun=.15;}}
 }
 ...
 if(primary.length){
  ...
  if(index===1&&syn(stats,'permafrost'))for(const e of enemies)if(!e.dead&&Math.hypot(e.x-player.x,e.z-player.z)<6)stopCast(e,e.kind===5?1:4);
