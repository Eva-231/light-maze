...
export const SPELLS=[{name:'光弾',key:'bolt',cost:6,cooldown:.65,description:'狙って放つ。外すとMPを消費。攻撃後の隙で威力上昇。'},{name:'氷環',key:'frost',cost:16,cooldown:0,description:'現在いる部屋全体を凍結爆破。部屋内の全敵へダメージ＋足止め。MPがある限り連続使用できる。'},{name:'治癒',key:'heal',cost:20,cooldown:0,description:'HPを30回復。MPがある限り連続使用できる。'}];
...
export function castSpell(index,enemies,player,stats,random=Math.random){
 ...
 let bounced=[];const selected=index===0?targets.slice(0,has(stats,'pierce')||has(stats,'drill')?3:has(stats,'twinstar')?2:1):index===1?targets:targets;if(index===0&&stats.echo&&selected.length){const first=selected[0],echo=enemies.filter(e=>!e.dead&&!e.escaped&&e!==first&&Math.hypot(e.x-first.x,e.z-first.z)<5&&lineOfSight(first.x,first.z,e.x,e.z)).slice(0,stats.echoCount||1);selected.push(...echo);}
