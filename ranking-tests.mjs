...
response=await api('/api/leaderboard/submit',{token:tokenA,method:'POST',body:{playerName:'月影改',run:normal({floor:8,trialLevel:12,time:340,parTime:360,kills:10})}});assert.equal(response.status,200);result=await response.json();assert.deepEqual(result.ranks.map(item=>item.board),['skill','abyss']);board=await (await api('/api/leaderboard?board=abyss',{token:tokenA})).json();assert.equal(board.own.trialLevel,12);assert.equal(board.own.rank,1);
