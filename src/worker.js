import {validRelicV10,validCache,relicSlots,validAbyss,validCodex} from '../public/save-schema.js';
...
export function validProgress(d){
 if(!d||![1,2,3,4,5,6,7,8].includes(d.format)||typeof d.mask!=='string'||d.mask.length>90000||!list(d.completed,13))return false;
 ...
 if(d.format>=4){if(!integer(d.bestSkill))return false;if(!d.chapterStars||typeof d.chapterStars!=='object'||Array.isArray(d.chapterStars)||Object.entries(d.chapterStars).some(([k,v])=>!(/^(?:[1-9]|1[0-8])$/.test(k))||!integer(v,1,3)))return false;
  if(!list(d.claimedGifts,8)||!d.claimedGifts.every(n=>integer(n,1,8))||!list(d.hintsSeen,100)||!d.hintsSeen.every(k=>typeof k==='string'&&/^[a-z0-9-]{1,40}$/.test(k)))return false;
 ...
 if(d.format>=8&&(!validAbyss(d.abyss)||!validCodex(d.codex)||!integer(d.shards)))return false;
...
async function parseBody(request,max=300000){if(!request.headers.get('content-type')?.startsWith('application/json'))return{error:json({error:'JSON required'},415)};if(Number(request.headers.get('content-length'))>max)return{error:json({error:'Too large'},413)};const raw=await request.text();if(raw.length>max)return{error:json({error:'Too large'},413)};try{return{raw,data:JSON.parse(raw)}}catch{return{error:json({error:'Invalid JSON'},400)}}}
...
export function competitiveScore(run){
 const accuracy=run.shots?Math.round(run.hits/run.shots*100):0,depth=run.mode==='daily'?1260:run.mode==='abyss'?1800+run.abyssFloor*140:run.floor*180;
 ...
 return Math.round(Object.values(parts).reduce((sum,value)=>sum+value,0)*(run.mode==='abyss'?1+(run.mutators?.length||0)*.25:1));
}
export function validRankRun(run){
 if(!run||run.rules!==8||run.cleared!==true||run.success!==true||!['normal','daily','abyss'].includes(run.mode)||!integer(run.seed,0,4294967295)||!integer(run.floor,1,18)||!integer(run.trialLevel,0,1000000)||!finite(run.time,1,run.mode==='abyss'?100000:5000)||!finite(run.parTime,10,run.mode==='abyss'?200000:2000)||!integer(run.kills,0,run.mode==='abyss'?20000:300)||!integer(run.shots,0,50000)||!integer(run.hits,0,run.shots)||!integer(run.weakHits,0,run.hits*20)||!integer(run.dodges,0,20000)||!integer(run.counterHits,0,run.hits*20)||!integer(run.maxHuntChain,0,run.kills)||!integer(run.interrupts,0,run.hits*20)||!finite(run.damageTaken,0,1000000))return false;
 if(run.mode==='abyss')return integer(run.abyssFloor,1,99)&&list(run.mutators||[],5)&&new Set(run.mutators||[]).size===(run.mutators||[]).length&&(run.mutators||[]).every(k=>['wrath','horde','famine','elite','night'].includes(k));
...
async function leaderboardData(env,board,identity){
 const period=board==='daily'?jstDay():board==='abyss'?'99f-v10':'campaign-v10',top=await env.DB.prepare('SELECT participant_id, player_name, score, elapsed_ms, trial_level, floor, rank_value, submitted_at FROM leaderboard_entries WHERE board = ? AND period = ? ORDER BY rank_value DESC, submitted_at ASC LIMIT 50').bind(board,period).all(),rows=top.results||[];
