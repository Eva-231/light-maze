...
test('Chapter stars, deferred one-time gifts, and campaign advancement survive replays and inventory pressure',()=>{
 ...
 p.floor=8;advanceJourney(p,{...run,floor:8,trialLevel:5},true);assert.equal(p.floor,9);assert.equal(p.bestTrial,0);assert.equal(p.trialLevel,1);const snapshot=structuredClone(p);advanceJourney(p,{...run,floor:8,trialLevel:6,runMode:'daily'},true);assert.deepEqual(p,snapshot);assert(validProgress(p));
});
